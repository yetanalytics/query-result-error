(defproject query-result-error "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "query-result-error.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.2"]]}
             :uberjar {:aot [query-result-error.server]}}
  :main ^{:skip-aot true} query-result-error.server)
